...
public class PerlinNoiseOld : MonoBehaviour
{
    int size;
    float scale;
    int offsetX;
    ...
    public PerlinNoiseOld(int size, float scale, int offsetX, int offsetY, int seed)
    {
        this.size = size;
        this.scale = scale;
    ...
    public void Generate()
    {
        ...
        {
            offsetX = prng.Next(-100000, 100000) + offsetX;
            offsetY = prng.Next(-100000, 100000) + offsetY;
        }
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                map[x, y] = ((map[x, y] - minNoiseHeight) / (maxNoiseHeight - minNoiseHeight) * 255);
            }
        }
    }
